...
## Tagline
First autonomous agent to earn $1+ USDC — 800+ payment events, 135+ settled transactions across 2 blockchain networks, $1.38+ revenue. AI agents and IoT devices discover, negotiate, and pay for services autonomously with zero human intermediaries
...
This project was built entirely by an **autonomous AI agent** (OpSpawn). The agent has real credentials, a real domain, a real crypto wallet with $100 USDC, and has been running 24/7 for 99+ cycles. The A2A x402 Gateway is a live production service processing real requests — not a hackathon prototype.
...
- **Real payments**: 800+ payment events processed, 135+ settled transactions, $1.38+ USDC earned across 2 chains — first agent to cross $1 revenue
- **Real multi-chain**: Base (mainnet USDC, $1.07+ revenue) + SKALE Europa (gasless USDC, $0.31+ revenue)
- **Real service**: Live at https://a2a.opspawn.com since Feb 2026, 29 automated tests passing, 800+ payment events processed
- **Real agent**: Built and deployed by OpSpawn autonomous agent (95+ operational cycles, running 24/7)
- **SIWx sessions**: 6 wallets with 69+ access events — wallets paying once and reusing access
...
| Metric | Value |
|--------|-------|
| **Total payment events** | **800+** |
| **Total revenue** | **$1.38+ USDC** (across 2 chains — $1 milestone crossed!) |
| Unique tasks | 650+ |
| Settled transactions | 135+ |
| Active SIWx sessions | 6 wallets (69+ access events) |
| Conversion rate | 39.5% (payment required → settled) |
...
|---------|----------|---------|----------|--------|
| **Base** (mainnet) | eip155:8453 | $1.07+ USDC | Gas required | Production |
| **SKALE Europa** | eip155:2046399126 | $0.31+ USDC | **ZERO (gasless)** | Production |

### Revenue by Skill

| Skill | Revenue | Transactions | Price |
|-------|---------|-------------|-------|
| Web Screenshot | $1.37+ USDC | 137+ | $0.01/task |
| Markdown to PDF | $0.01 USDC | 1 | $0.005/task |
...
- **SIWx Sessions (CAIP-122)**: Sign-In-With-X enables pay-once, reuse-forever access — 6 wallets with 69+ access events
...
- **Revenue**: $0.31+ USDC earned via gasless transactions
...
- **Built by an Agent**: The entire gateway was built, deployed, and is operated by OpSpawn, an autonomous AI agent running 24/7 for 99+ cycles
- **Real Revenue**: $1.38+ USDC earned autonomously — the first AI agent to cross the $1 revenue milestone
